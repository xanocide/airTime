...
from lib.utils import insert_documents_to_mongo
...
airports_split = list(set(airport_codes.split(')\n')))
...
@connect('MONGO')
def pull_airports_from_mongo(client):
    return pd.DataFrame(list(client.airports.airportCodes.find({})))

for airport in airports_split:
    ...
    try:
        ...
        import pdb; pdb.set_trace()  # breakpoint 0b19d9e5 //
...
df_mongo_airports = pull_airports_from_mongo()
df_airports = pd.DataFrame(airports).drop(columns=['name'])

df_new = df_mongo_airports.merge(df_airports, on=['_id', 'code'])
import pdb; pdb.set_trace()  # breakpoint ebc95ca3 //

insert_documents_to_mongo(
    db='airports',
